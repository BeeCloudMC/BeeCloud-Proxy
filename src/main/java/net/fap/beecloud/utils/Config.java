...
public class Config {
	...
	public boolean contains(String index) {
		return getConfigValue(index) != null;
	}
	...
	private String getConfigValue(String index) {
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(this.configFilePath), StandardCharsets.UTF_8));
	...
	private void writeData(String data) {
		try {
			...
			if (contains(data.split("\\:")[0]))
				remove(data.split("\\:")[0]);
			BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(configFilePath, true), StandardCharsets.UTF_8));
